...
(function (window, document, undefined) {
    ...
    var log = function (msg) {
        ...
        removeElements = function (remove) {
            for (var i = 0, len = remove.length; i < len; i++) {
                var item;
                if ( remove instanceof HTMLCollection ) item = remove[0];
                else item = Array.prototype.pop.call( remove );
                if (item !== undefined) {
    ...
    window.Feedback = function (options) {
        ...
        // default properties
        options.label = options.label || "Geribildirim";
        options.header = options.header || "Görüşlerinizi Gönderin";
        options.url = options.url || "/";
        options.adapter = options.adapter || new window.Feedback.XHR(options.url);

        options.data = options.data || {};

        options.nextLabel = options.nextLabel || "Devam";
        options.reviewLabel = options.reviewLabel || "Önizleme";
        options.sendLabel = options.sendLabel || "Gönder";
        options.closeLabel = options.closeLabel || "Kapat";

        options.messageSuccess = options.messageSuccess || "Görüşleriniz başarıyla gönderildi.";
        options.messageError = options.messageError || "Sunucuya görüşlerinizi gönderirken bir hata oluştu.";

    		options.blackoutLabel = options.blackoutLabel || "Karart";
    		options.highlightLabel = options.highlightLabel || "Öne Çıkar";
    		options.highlightOrBlackoutMessage = options.highlightOrBlackoutMessage || "Lütfen Karartma veya Öne Çıkarma işlemini yapın!";
        ...
        var button,
            ...
            returnMethods = {
                ...
                send: function (adapter) {
                    ...
                    adapter.send(options.data, data, function (success) {
        ...
        button.className = "feedback-btn feedback-bottom-left";
    ...
    window.Feedback.Form = function (elements) {
        ...
        this.elements = elements || [{
            type: "textarea",
            name: "Konu",
            label: "Lütfen Görüşlerinizi Açıklayın.",
            required: false
    }];
    ...
    window.Feedback.XHR.prototype.send = function (extradata, data, callback) {

        var xhr = this.xhr;

        var postData = "data=" + encodeURIComponent(window.JSON.stringify(data));

        for (prop in extradata) {
          if (extradata.hasOwnProperty(prop)) {
            postData+= "&"+prop+"="+encodeURIComponent(extradata[prop]);
          }
        }
